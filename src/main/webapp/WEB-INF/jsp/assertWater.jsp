...
<script type="text/javascript">
    ...
    function addComment() {
        ...
        $.post("<%=basePath%>assertWater/changeState.action", $("#edit_comment_form").serialize(), function (data) {
            if (data.code == 0) {
                alert(data.msg);
                window.location.reload();
            } else {
    ...
    function submit(id) {
        if (confirm('确定要提交审核吗?')) {
            $.ajax({
                type: "get",
                url: "<%=basePath%>assertWater/submit.action",
                data: {"id": id},
                success: function (data) {
                    if (data.code == 0) {
                        alert(data.msg);
    ...
    function comments(id) {
        $.ajax({
            type: "get",
            url: "<%=basePath%>assertWater/getComments.action",
            data: {"id": id},
            success: function (data) {
                if (data.code == 0) {
