                        ...
                        <c:forEach items="${page.rows}" var="row">
                            <tr>
                                ...
                                <td>
                                    ...
                                    <!--  租金录入模块 -->
                                    <security:authorize access="hasAnyRole('ROLE_RENTAL')">
                                    ...
                                    <!--  租金审核模块 -->
                                    <security:authorize access="hasAnyRole('ROLE_RENTALCHECK')">
                                        <a href="#" class="btn btn-success btn-xs" data-toggle="modal"
                                           data-target="#showDialog"
                                           onclick="comments(${row.id})">查看审核记录</a>
            ...
            <div class="modal-body">
                <form class="form-horizontal" id="edit_comment_form">
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-10">
                            <select class="form-control" id="edit_comment_state" placeholder="是否通过"
                                    name="comment_state">
...
<script type="text/javascript">
    ...
    function addComment() {
        ...
        $.post("<%=basePath%>assertRental/changeState.action", $("#edit_comment_form").serialize(), function (data) {
            if (data.code == 0) {
                alert(data.msg);
                window.location.reload();
            } else {
    ...
    function submit(id) {
        if (confirm('确定要提交审核吗?')) {
            $.ajax({
                type: "get",
                url: "<%=basePath%>assertRental/submit.action",
                data: {"id": id},
                success: function (data) {
                    if (data.code == 0) {
                        alert(data.msg);
    ...
    function comments(id) {
        $.ajax({
            type: "get",
            url: "<%=basePath%>assertRental/getComments.action",
            data: {"id": id},
            success: function (data) {
                if (data.code == 0) {
